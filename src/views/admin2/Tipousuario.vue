...
onMounted(() => {
    axios.get('http://localhost/asitenback/public/api/v1/areas')
        .then((response) => {
...
const guardararea = () => {
    ...
    if (area.value.nombre_area) {
        if (area.value.id) {
            axios.put('http://localhost/asitenback/public/api/v1/areas/' + area.value.id, {
            ...
            axios.post('http://localhost/asitenback/public/api/v1/areas',
            { 
                    nombre_area: area.value.nombre_area,
                    sigla: area.value.nombre_area.substring(0,3)
                }).then((result) => {
...
const deletearea = () => {
    ...
    deleteareaDialog.value = false;
            axios.delete(`http://localhost/asitenback/public/api/v1/areas/${area.value.id}`)
                .then((response) => {
