...
onMounted(() => {
    axios
        .get(`${import.meta.env.API_BASE_URL}/vacaciones`)
        .then((response) => {
            const apiData1 = response.data;
            console.log(response.data);
            vacaciones.value = apiData1;
        })
        .catch((error) => {
            toast.add({ severity: 'error', summary: 'Error', detail: 'No se pudo cargar la information de vacaciones', life: 3000 });
            console.log(error);
        });
    axios
...
watch(personaSeleccionada, (newVal) => {
    nombreSeleccionado.value = obtenerNombrePorCodId(newVal);
});
            ...
            <div class="card">
                ...
                <Toolbar class="mb-4">
                    <template v-slot:start>
                        <div class="my-2">
                            <Button label="Asignar nueva Vacacion" icon="pi pi-plus" class="p-button-success mr-2" @click="openNew" />
                        </div>
                    </template>
                </Toolbar>
                <DataTable
                ...
                >
                    <template #header>
                        <div class="flex flex-column md:flex-row md:justify-content-between md:align-items-center">
                            <h5 class="m-0">LISTAR PERSONAL CON VACACIONES</h5>
                    ...
                    <Column field="expedido" header="FECHA SOLICITUD" :sortable="true" headerStyle="width:14%; min-width:10rem;">
                        <template #body="slotProps">
                            <span class="p-column-title">FECHA SOLICITUD</span>
                            {{ slotProps.data.expedido }}
                        </template>
                    </Column>
                    <Column field="nombres" header="FECHA SALIDA" :sortable="true" headerStyle="width:14%; min-width:10rem;">
                        <template #body="slotProps">
                            <span class="p-column-title">FECHA SALIDA</span>
                            {{ slotProps.data.nombres }}
                        </template>
                    </Column>
                    <Column field="ap_paterno" header="FECHA LLEGADA" :sortable="true" headerStyle="width:14%; min-width:8rem;">
                        <template #body="slotProps">
                            <span class="p-column-title">FECHA LLEGADA</span>
                            {{ slotProps.data.ap_paterno }}
                        </template>
                    </Column>
                    <Column field="ap_materno" header="DIAS" :sortable="true" headerStyle="width:14%; min-width:8rem;">
                        <template #body="slotProps">
                            <span class="p-column-title">DIAS</span>
                            {{ slotProps.data.ap_materno }}
                        </template>
                    </Column>
                    <Column field="fecha_nac" header="DEPENDENCIA" :sortable="true" headerStyle="width:14%; min-width:8rem;">
                        <template #body="slotProps">
                            <span class="p-column-title">DEPENDENCIA</span>
                            {{ slotProps.data.fecha_nac }}
                        </template>
                    </Column>
                    <Column field="nacionalidad" header="OBSERVACIONES" :sortable="true" headerStyle="width:14%; min-width:8rem;">
                        <template #body="slotProps">
                ...
                <Dialog v-model:visible="personalDialog" :style="{ width: '70rem' }" header="Personal Detalle" :modal="true" class="p-fluid">
                    <div>
                        ...
                        <!-- Mostrar el nombre en un campo de texto -->
                        <div>
                            <label for="nombreSeleccionado">Nombre Seleccionado:</label>
                            <input type="text" v-model="nombreSeleccionado" readonly />
                        </div>
                    </div>
